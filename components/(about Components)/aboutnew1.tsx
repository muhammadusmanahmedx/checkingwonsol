        ...
        <div className="grid max-w-md grid-cols-1 mx-auto lg:grid-cols-12 gap-x-8 gap-y-6 lg:max-w-none">
          <div className="self-center lg:col-span-4">
            <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl xl:text-4xl leading-tight">
              Welcome to 
              <span className="text-blue-600"> Wonsol</span> 
                {" "}Innovating Software Solutions
            </h1>
            <p className="mt-3 text-sm font-normal leading-6 text-gray-500 sm:text-base sm:leading-7">
              We are a cutting-edge software development company dedicated to creating innovative solutions 
              that transform businesses and drive digital success. Our passion for technology meets your vision for growth.
            </p>
          ...
          <div className="self-end lg:order-last lg:pb-16 lg:col-span-3">
            <p className="text-xs font-bold tracking-widest text-gray-500 uppercase">⭐ Our Achievements</p>

            <div className="mt-4 space-y-4 lg:space-y-5">
              {achievements.map((achievement, index) => (
                <div key={index} className="relative overflow-hidden">
                  <div className="flex items-start lg:items-center">
                    <div className="flex items-center justify-center w-10 h-10 rounded-lg shrink-0" style={{backgroundColor: '#2C74BC'}}>
                      <span className="text-white font-bold text-sm">{achievement.value}</span>
                    </div>
                    <div className="ml-4">
          ...
          <div className="self-end lg:col-span-5">
            <div className="relative max-w-xs mx-auto lg:max-w-sm">
              ...
              <div className="absolute -top-4 -right-4 w-20 h-20 rounded-full opacity-20" style={{backgroundColor: '#2C74BC'}}></div>
              <div className="absolute -bottom-6 -left-6 w-16 h-16 rounded-full opacity-10" style={{backgroundColor: '#2C74BC'}}></div>
              
              {/* Main image container with border */}
              <div className="relative bg-white rounded-2xl p-2 shadow-lg">
                <div className="rounded-xl overflow-hidden border-2" style={{borderColor: '#2C74BC'}}>
                  <img 
                    className="w-full h-64 sm:h-72 md:h-80 object-cover" 
                    src={companyImage} 
                    alt="Wonsol Software Company" 
                  />
                </div>
                
                {/* Floating badge */}
                <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full text-white text-sm font-semibold shadow-lg" style={{backgroundColor: '#2C74BC'}}>
                  Est. 2024
                </div>
              </div>
              
              {/* Side accent elements */}
              <div className="absolute top-1/2 -left-2 w-1 h-16 rounded-full opacity-60" style={{backgroundColor: '#2C74BC'}}></div>
              <div className="absolute top-1/3 -right-2 w-1 h-12 rounded-full opacity-40" style={{backgroundColor: '#2C74BC'}}></div>
            </div>
          </div>
        </div>
      </div>
